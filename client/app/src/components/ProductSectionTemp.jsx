...
const ProductSection = ({ type, title }) => {
  ...
  const handleAddtoCart = (item) => {
    ...
    if (item.variants && item.variants.length > 0) {
      selectedVariant = item.variants.find(v => (v.countInStock || 0) > 0) || item.variants[0];
    }
    dispatch(addToCart({ ...item, selectedVariant }));
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.map((item) => {
            const isOutOfStock = item.variants && item.variants.length > 0
              ? item.variants.reduce((sum, v) => sum + (v.countInStock || 0), 0) === 0
              : false;
                ...
                <Link to={`/product/${item._id}`} className="block">
                  ...
                  <div className="relative overflow-hidden h-64 bg-linear-to-br from-gray-50 to-gray-100">
                    ...
                    <button
                      ...
                      className={`absolute top-4 left-4 p-2 rounded-full transition-all shadow-md z-10 ${
                        isInCompare(item._id)
                          ? "bg-green-500 text-white"
                          : "bg-white text-gray-600 hover:text-green-600"
                      }`}
                      title={isInCompare(item._id) ? "Remove from comparison" : "Add to comparison"}
                    >
                ...
                <div className="p-6 pt-0">
                  <div className="flex gap-3">
                    <button
                      ...
                      className={`flex-1 py-3 px-4 rounded-lg transition-all duration-300 transform font-semibold cursor-pointer ${
                        isOutOfStock
                          ? "bg-gray-200 text-gray-400 cursor-not-allowed scale-100"
                          : "bg-linear-to-r from-orange-500 to-red-500 text-white hover:from-orange-600 hover:to-red-600 hover:scale-105"
                      }`}
                    >
